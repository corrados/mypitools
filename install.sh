...
# EXTERNAL USB HDD #############################################################
# DOES NOT WORK CORRECTLY, right now I have defaults,nofail,noauto and mount manually
if grep -Fxq "/dev/sda1 /media/piarchiv ext4 defaults,nofail,noauto 0 0" /etc/fstab
then
	...
	sudo echo "/dev/sda1 /media/piarchiv ext4 defaults,nofail,noauto 0 0" | sudo tee -a /etc/fstab >/dev/null
