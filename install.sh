...
if [[ ! -v is_media ]]; then
  ...
  0   3    1 * *       sudo reboot
  0   7    * * 1       sudo apt update && sudo apt dist-upgrade -y"
else
  CRON_TABLE="0 7 * * 1 sudo apt update && sudo apt dist-upgrade -y"
fi
...
sudo apt-get install feh tvheadend gnuplot build-essential fail2ban git hdparm htop net-tools nethogs pigpio vim -y
